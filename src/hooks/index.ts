// Custom hooks for the Pacman game
// Individual hooks will be implemented in later tasks

export * from './useLocalStorage';
export * from './useGameContext';
export * from './useCollectibles';
export * from './useMovement';
export * from './useSimpleMovement';
export * from './useCollisionDetection';
export * from './useScore';
export * from './useGameCollision';
export * from './useGhostAI';
export * from './usePowerMode';
export * from './useGhostConsumption';
export * from './usePowerPelletSystem';
// Additional hooks will be exported as they are created
