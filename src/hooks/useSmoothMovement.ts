...
interface UseSmoothMovementOptions {
  ...
  speed?: number; // Movement duration in milliseconds
...
export const useSmoothMovement = ({
  ...
  // Set the movement direction with improved queuing
  const setDirection = useCallback(
    (newDirection: Direction) => {
      setMovementState(prev => {
        // If we can immediately move in the new direction
    ...
    intervalRef.current = window.setInterval(() => {
      setMovementState(prev => {
        ...
        if (canMove(prev.gridPosition, currentDirection)) {
          ...
          animateToPosition(prev.visualPosition, nextGridPos, speed * 0.8); // Use 80% of speed for smoother feel
